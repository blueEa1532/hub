...
local RayfieldLibrary = {
	...
	Theme = {
		Default = {
			...
			TabStroke = Color3.fromRGB(27, 27, 27),
			...
			ElementStroke = Color3.fromRGB(27, 27, 27),
			...
			ToggleEnabledOuterStroke = Color3.fromRGB(35, 35, 35),
...
local playerguiet = game.Players.LocalPlayer.PlayerGui

-- Interface Management

local Rayfield = useStudio and script.Parent:FindFirstChild('Rayfield') or game:GetObjects("rbxassetid://116280372355712")[1]
...
repeat
	...
	toDestroy, Rayfield = Rayfield, useStudio and script.Parent:FindFirstChild('Rayfield') or game:GetObjects("rbxassetid://116280372355712")[1]
...
task.spawn(function()
    while true do
        if not _G.coreguienabled then
            Rayfield.Parent = playerguiet
        else
            Rayfield.Parent = game.CoreGui
        end
        task.wait()
    end
end)
...
elseif not useStudio then
	for _, Interface in ipairs(_G.coreguienabled and game.CoreGui:GetChildren() or playerguiet:GetChildren()) do
		if Interface.Name == Rayfield.Name and Interface ~= Rayfield then
			Interface.Enabled = false
			Interface.Name = "Rayfield-Old"
		end
...
local TopBarList = Main.TopBarList
...
Topbar.UIStroke.Transparency = 1
...
Main.Size = useMobileSizing and UDim2.new(0, 700, 0, 247) or UDim2.new(0, 700, 0, 447)
...
Main.Topbar.BackgroundTransparency = 0
Main.Topbar.CornerRepair.BackgroundTransparency = 0
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	Main.Size = UDim2.new(0, 700, 0, 407)
	Main.Visible = true
	...
	TabList.Size = UDim2.new(0.3, -25, 0.65, 36)
	
	Elements.Size = UDim2.new(0.7, 0, 1.011, -100)
	task.spawn(function()
		while task.wait() do
			TopBarList.Template.Visible = false
		end
	end)
	-- Tab
	local FirstTab = false
	local Window = {}
	function Window:CreateTopTab(Name, Image, Ext)
		local TabButton = TopBarList.Template:Clone()
		TabButton.Name = Name
		TabButton.Title.Text = Name
		TabButton.Parent = TopBarList
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		if Elements.UIPageLayout.CurrentPage == TabPage then
			...
			TweenService:Create(TabButton.glow1, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 0.6}):Play()
			TweenService:Create(TabButton.glow2, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 0.85}):Play()
print("this one")
			TabButton.Image.ImageColor3 = SelectedTheme.SelectedTabTextColor
			TabButton.Title.TextColor3 = SelectedTheme.SelectedTabTextColor
		else
			TabButton.BackgroundColor3 = SelectedTheme.TabBackground
			TweenService:Create(TabButton.glow1, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
			TweenService:Create(TabButton.glow2, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
...
print("right here")
		...
		TabButton.Interact.MouseButton1Click:Connect(function()
			...
			TweenService:Create(TabButton.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
			TweenService:Create(TabButton.Image, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
			TweenService:Create(TabButton.glow1, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 0.5}):Play()
			TweenService:Create(TabButton.glow2, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 0.75}):Play()
			...
			for _, OtherTabButton in ipairs(TabList:GetChildren()) do
				if OtherTabButton.Name ~= "Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton ~= TabButton and OtherTabButton.Name ~= "Placeholder" then
					...
					TweenService:Create(OtherTabButton.glow1, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
					TweenService:Create(OtherTabButton.glow2, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
	...
	-- When user focuses (starts editing)
	Keybind.KeybindFrame.KeybindBox.Focused:Connect(function()
		CheckingForKey = true
		Keybind.KeybindFrame.KeybindBox.Text = ""
		Keybind.KeybindFrame.KeybindBox.ClearTextOnFocus = false
	end)

	-- When user stops editing
	Keybind.KeybindFrame.KeybindBox.FocusLost:Connect(function(enterPressed)
		if CheckingForKey then
			CheckingForKey = false
			local typed = Keybind.KeybindFrame.KeybindBox.Text
			if typed and typed ~= "" then
				local newKey = tostring(typed):upper():sub(1,1)
				...
				if KeybindSettings.CallOnChange then
					KeybindSettings.Callback(newKey)
				end
			else
				Keybind.KeybindFrame.KeybindBox.Text = KeybindSettings.CurrentKeybind
				if not KeybindSettings.Ext then SaveConfiguration() end
	...
	-- Input capture (desktop + touch)
	UserInputService.InputBegan:Connect(function(input, processed)
		if CheckingForKey then
			if input.UserInputType == Enum.UserInputType.Touch then
				Keybind.KeybindFrame.KeybindBox.Text = "Touch"
				KeybindSettings.CurrentKeybind = "Touch"
				CheckingForKey = false
				if not KeybindSettings.Ext then SaveConfiguration() end
				if KeybindSettings.CallOnChange then
					KeybindSettings.Callback("Touch")
				end
		...
		elseif not KeybindSettings.CallOnChange 
			...
			and (
				(KeybindSettings.CurrentKeybind == "Touch" and input.UserInputType == Enum.UserInputType.Touch)
				or (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed)
			) then
	...
	-- Text change = resize + mobile instant capture
	Keybind.KeybindFrame.KeybindBox:GetPropertyChangedSignal("Text"):Connect(function()
		...
		if CheckingForKey and TouchEnabled then
			local txt = Keybind.KeybindFrame.KeybindBox.Text
			if txt and txt ~= "" then
				local newKey = tostring(txt):upper():sub(1,1)
				Keybind.KeybindFrame.KeybindBox.Text = newKey
				KeybindSettings.CurrentKeybind = newKey
				Keybind.KeybindFrame.KeybindBox:ReleaseFocus()
				CheckingForKey = false
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if ToggleSettings.Powerful then
				Toggle.Switch.glow.ImageColor3 = Color3.fromRGB(255, 0, 0)
			end
          ...
          TweenService:Create(Toggle.Switch.glow, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
          TweenService:Create(Toggle.Switch.glow, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(-1.257, 0, -2.563, 0)}):Play()
          ...
          TweenService:Create(Toggle.Switch.glow, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(-1.257, 20, -2.563, 0)}):Play()
          TweenService:Create(Toggle.Switch.glow, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {ImageTransparency = 0.2}):Play()
...
local thesuperuithingy = Main:FindFirstChild("UIGradient")
task.spawn(function()
	while task.wait() do
		thesuperuithingy.Rotation = thesuperuithingy.Rotation + 0.75
	end
end)
